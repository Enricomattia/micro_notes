...
\begin{techremark}
	Sometimes \usename{axn:wo} is referred to as \textbf{rationality} of preferences (see e.g. \citet[p.~6]{mas-colellMicroeconomicTheory1995}). However, I think this is an unfortunate name. It suggests that it is \enquote{irrational} to violate \usename{axn:wo}, but there are reasons why people might have intransitive or incomplete preferences (can you think of any?). An interesting discussion on the relationship between rationality and \textit{intelligence} is in \citet[ch. 1]{myersonGameTheoryAnalysis1997}.
\end{techremark}
...
\begin{proof}
	...
	By the inductive hypothesis, \(\overline p \succsim q\); and by \textbf{Step 1}, \(\overline p \succsim \delta_x\). We apply \usename{axn:independence} twice. From \(\overline p \succsim q\), mix with \(\overline p\):
	\[
...
\begin{proof}
	...
	\paragraph{Step 4.} Define a utility function \( U \colon \Delta(X) \to \mathbb{R} \) that assigns to each lottery a number representing its utility, defined by \( U(p) = \alpha_p \). This function represents preferences \( \succsim \).
...
In Exercise~\ref{ex:diff}, you are asked to elaborate on the significance of Corollary~\ref{cor:uniq} for interpreting utility numbers as \emph{cardinal} representations of preferences.\footnote{If you are interested in this aspect, you may read \citet[ch. 3]{vonneumannTheoryGamesEconomic2007}. An illuminating discussion of measurement in the social sciences is in \citet[ch. 1]{krantzFoundationsMeasurementVol1971}.}
...
\begin{exercise}\label{ex:indlin}
	Explain why the fact that indifference curves are straight and parallel lines follows from \usename{axn:independence}. I.e., use the axiom, not the functional form!
\end{exercise}
...
\begin{exercise}
	Revisit the choice problem in Exercise~\ref{ex:allais}. Show that the preferences exhibited there do not satisfy \usename{axn:independence}. Representing the lotteries in a table might help.
\end{exercise}

\begin{exercise}
	Consider the \textbf{Betweenness Axiom} introduced by \citet{dekelAxiomaticCharacterizationPreferences1986}: for all lotteries \( p, q \) and \( \alpha \in [0,1] \), if \( p \sim q \), then \( \alpha p + (1-\alpha) q \sim p \). Show that \usename{axn:independence} implies Betweenness, but Betweenness does not imply \usename{axn:independence}. (Hint: for the second part, construct a preference relation that satisfies Betweenness but not \usename{axn:independence}, maybe in the graph.) Are indifference curves still linear under Betweenness? Are they parallel?
\end{exercise}
...
\begin{exercise}\label{ex:diff}
	Explain why Corollary~\ref{cor:uniq} allows us to make statements such as \textquote{the difference in utility between lottery \( p \) and lottery \( q \) is greater than the difference in utility between lottery \( r \) and lottery \( s \)}. Why this would not be possible without Corollary~\ref{cor:uniq}?
\end{exercise}
